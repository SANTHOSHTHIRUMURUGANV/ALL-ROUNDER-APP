...
// Verify that phone is one of the 3 approved admin phone numbers
const isApprovedPhone = (phone) => {
  const approved = [
    (process.env.ADMIN_PHONE_1 || '8668167035'),
    (process.env.ADMIN_PHONE_2 || '6379999080'),
    (process.env.ADMIN_PHONE_3 || '7010535942')
  ];
  const cleanPhone = String(phone).replace(/\D/g, '');
  return approved.includes(cleanPhone);
};

// Send OTP via Email and SMS
const deliverOtp = async (email, phone, otp) => {
  // 1. SMTP Email Delivery if configured
  ...
  // 2. SMS Delivery via Twilio if configured
  if (phone) {
    const twilioClient = getTwilioClient();
    if (twilioClient && process.env.TWILIO_PHONE_NUMBER) {
      try {
        await twilioClient.messages.create({
          body: `Your AllCounter admin verification code is: ${otp}. This code will expire in 5 minutes.`,
          from: process.env.TWILIO_PHONE_NUMBER,
          to: `+91${phone}` // Assuming Indian phone numbers
        });
        console.log('📱 SMS OTP sent to:', phone);
      } catch (smsErr) {
        console.warn("SMS delivery failed:", smsErr.message);
      }
    }
  }

  // 3. Write to backend/otp-debug.log file (Excludes console logs/DB/API disclosure)
  try {
    const logLine = `[${new Date().toISOString()}] Email: ${email} | Phone: ${phone} | OTP: ${otp}\n`;
...
export const loginAdmin = async (req, res, next) => {
  try {
    const { userNumber, password, email, phone } = req.body || {};

    if (!userNumber || !password || !email || !phone) {
    ...
    // Deliver OTP via Email and SMS
    await deliverOtp(email, phone, otp);

    res.status(200).json({
      success: true,
      message: 'OTP passcode sent to your email and phone.',
...
export const resendAdminOtp = async (req, res, next) => {
  try {
    ...
    await deliverOtp(email, record.phone, otp);

    res.status(200).json({
      success: true,
      message: 'A new OTP passcode was dispatched to your email and phone.'
    });
  } catch (error) {
    next(error);
  }
